...
V - Guest can appear on many episodes
V - Episode can have multiple guests
V - Appearance stores a numeric rating

### 2. New Appearance Form

A user can fill out a form to create a new Appearance. They can:

 V- Choose an existing guest from a select dropdown
 V- Choose an existing episode from a select dropdown
V- Enter a numeric rating
V- Submit the form

 V After submitting the form, the user should be redirected to the selected episode's show page.
...
V  - Episode date
 V- A list of the guests who were on that episode
 V- Each guest's name should link to the Guest Show page
...
For each Episode, show the:
 ...
 V- date of the Episode
V - rating for the Appearance

V Each Episode date should link to the show page for that Episode.

### 5. Appearance Rating Validation

The rating on an Appearance should be between 1 and 5 (inclusive - `1` and `5` are okay).

V- Add a validation to ensure that the rating is between 1 and 5.
V- Add handling for this error to the Appearance create action.
V - The validation error should be shown on the Appearance creation form when a user attempts to save an appearance with an invalid rating.
